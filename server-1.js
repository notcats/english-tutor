...
async function initDB() {
  await pool.query(`
    ...
    -- Global word cache - shared between all users
...
app.get('/api/admin/stats', adminAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      top_words: topWords.rows,
      recent_users: recentUsers.rows,
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// PATCH /api/admin/user/:id — change role and limit
...
// DELETE /api/admin/cache/:id — delete cache entry
...
// Admin: set user role and limit
app.patch('/api/admin/user/:id', auth, async (req, res) => {
  const admin = await pool.query('SELECT role FROM users WHERE id=$1', [req.user.id]);
  if (admin.rows[0]?.role !== 'admin') return res.status(403).json({ error: 'Admin only' });
  const { role, daily_limit } = req.body;
  await pool.query('UPDATE users SET role=COALESCE($1,role), daily_limit=COALESCE($2,daily_limit) WHERE id=$3', [role, daily_limit, req.params.id]);
  res.json({ ok: true });
});
...
app.post('/api/ai/word', auth, aiLimit, async (req, res) => {
  try {
    ...
    const cached = await pool.query(
      'SELECT * FROM word_cache WHERE word=$1 AND learn_lang=$2 AND native_lang=$3',
      [wordLower, langs.learn_lang, langs.native_lang]
    );
    if (cached.rows.length > 0) {
      const c = cached.rows[0];
      console.log('Cache hit:', wordLower);
    ...
    // Not in cache - call AI and check limit
    const u = await pool.query('SELECT daily_used, daily_limit, last_reset FROM users WHERE id=$1', [req.user.id]);
    const user = u.rows[0];
    const today = new Date().toISOString().split('T')[0];
    const lastReset = user.last_reset?.toISOString?.()?.split('T')[0];
